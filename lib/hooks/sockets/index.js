	...
	return {
		...
		routes: {
			...
			before: {
				...
				'get /firehose': function(req, res) {
					if (!req.isSocket) {
						sails.log.error("Cannot subscribe to firehose over HTTP!  Firehose is for socket messages only.");
						return;
					}
					if (process.env.NODE_ENV !== 'development') {
						sails.log.warn("Subscribing to firehose is only recommended for apps in development.  Use model pubsub methods in your production app.");
					}
					sails.log.silly("Subscribed to firehose.");
